...
function calculateInterest() {
  const principal = parseFloat(document.getElementById("num1").value);
  const rate = parseFloat(document.getElementById("num2").value);
  const time = parseFloat(prompt("Enter time in years:"));
  const interest = (principal * rate * time) / 100;
  const totalAmount = principal + interest;
  document.getElementById("result").innerText = `Interest: ${interest.toFixed(
    2
...
function calculateRemainder() {
  ...
  document.getElementById("result").innerText = `Remainder: ${remainder}`;
}
